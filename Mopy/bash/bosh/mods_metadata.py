    ...
    class RuleParser:
        ...
        def __init__(self,ruleSet):
            if self.__class__.reMod is None: # bush.game must have been set
                espmls = '|'.join(map(re.escape, bush.game.espm_extensions))
                self.__class__.reMod = re.compile(
                    '' r'\s*([\-|]?)(.+?(' + espmls + r'))(\s*\[[^\]]\])?',
                    re.I | re.U)
                self.__class__.reModVersion = re.compile(
                    '(.+(' + espmls + r'))\s*(\[[^\]]+\])?', re.I | re.U)
...
class ConfigHelpers:
    ...
    def get_tags_from_dir(self, plugin_name):
        ...
        tag_file = tag_files_dir.join(plugin_name.body + '.txt')
        ...
        with tag_file.open('r') as ins:
            for tag_line in ins:
                ...
                tag_line = tag_line.split('#')[0].strip()
                if not tag_line: continue
                for tag_entry in tag_line.split(','):
    ...
    def save_tags_to_dir(self, plugin_name, plugin_tags, plugin_old_tags):
        ...
        diff_tags = sorted(plugin_tags - plugin_old_tags |
                           {'-' + t for t in plugin_old_tags - plugin_tags},
        ...
        with tag_file.open('w') as out:
            ...
            out.write('# Generated by Wrye Bash %s\n' % bass.AppVersion)
            out.write(', '.join(diff_tags) + '\n')
    ...
    _cleaning_wiki_url = '[[!https://tes5edit.github.io/docs/5-mod-cleaning' \
                         '-and-error-checking.html|Tome of xEdit]]'
    ...
    def checkMods(self, showModList=False, showRuleSets=False, showNotes=False,
        ...
        with sio() as out:
            ...
            if bush.game.check_esl:
                log(_('This is a report on your currently installed or '
                      'active mods.'))
            else:
                log(_('This is a report on your currently installed, active, '
                      'or merged mods.'))
            ...
            if shouldMerge:
                if bush.game.check_esl:
                    ...
                    log(_('Following mods could be assigned an ESL flag but '
                          'are not ESL flagged.'))
                else:
                ...
                for mod in sorted(shouldMerge):
                    log('* __'+mod.s+'__')
            if removeEslFlag:
                log.setHeader('=== ' + _('Potentially Incorrect ESL Flag'))
                log(_('Following mods have an ESL flag, but may not qualify. '
                      'Run \'Check ESL Qualifications\' on them and/or check '
                      'them with xEdit to be sure.'))
            ...
            if invalidVersion:
                ...
                log(_("Following mods have a TES4 version that isn't "
                      "recognized as (one of) the standard version(s) "
                      "(%s).  It is untested what effect this can have on "
                      "%s.") % (ver_list, bush.game.displayName))
